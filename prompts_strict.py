...
task_prompt = f"""\
# Social Determinants of Health (SDoH) Identification Task
You're an expert social worker specialized in identifying social determinants of health (SDoH) in clinical notes.
Your task is to analyze provided clinical text and identify relevant SDoH categories and sub-categories based ONLY on the explicit information in the text. 
...
2. Identify SDoH *categories* from the list above are explicitly present. Multiple selections are allowed. 
   • If no relevant categories are mentioned, include only the string "none" in your categories list.
...
3. Identify SDoH *sub-categories* from the list above that correspond to the chosen categories. Multiple selections are allowed. 
   ...
   • If no relevant sub-categories are mentioned, include only the string "none" in your sub-categories list.

4. Only include the SDoH categories and sub-categories that meet ALL of the following criteria:
	• *Current* - Reflects the patient's present situation
   		• Exclude events that are past conditions (e.g., "was homeless as a child") or resolved situations.
	• *Patient-specific* - Directly about the patient, not family members or others.
		• Exclude example: "Patient's brother lost his job".
    • *Actual* - Represents a real, observed status, not a plan, intention, or recommendation.
		• Exclude hypothetical scenarios, future plans, prospect, intentions, or recommendations, such as:
        	• "Submitted a housing application" (future possibility)
        	• "Pt is motivated and will attempt to be more active"
        	• "If all goes well pt will likely move into a stable house"
        	• "Recommended/discussed that pt increase physical activity"
    • *Confirmed* - Explicitly confirmed in the text, without uncertainty or pending status.
		• Exclude statements expressing uncertainty or ambiguity, such as:
			• SDoH question left unanswered (e.g., empty checkboxes) or marked “Pended”
			• "Pt is worried about possible eviction" (concern only)
			• "It may be the case that pt is homeless" (speculative)
...
(Think step-by-step internally if useful, but the user should see only the identified SDoH categories and sub-categories.)
"""
